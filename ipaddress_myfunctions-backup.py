...
def ips_para_hsrp(network, tres_primeiros=True, tres_ultimos=False):
    ...
    ultimoip, penultimoip, antepenultimoip = net.broadcast_address - 1, net.broadcast_address - 2, net.broadcast_address - 3

    if tres_primeiros == True and tres_ultimos == True:
        return str(mask), str(primeiroip), str(segundoip), str(tercip), str(antepenultimoip), str(penultimoip), str(ultimoip)
